...
#else
// C translation of asm_080343b8.s, the one function this engine still has in
// assembly. Without it the PC build has no marcher animation at all and the
// scene draws an empty parade ground.
void func_080343b8(struct Marcher *marcher, u32 action) {
    ...
    switch (action) {
        case 0:  // stand, holding the pose rather than playing it
        ...
        case 3:  // face left: mid-step, only the head turns
            if (marcher->idleTimer != 0) {
                newIdleTimer = marcher->idleTimer;
                showHead = TRUE;
            } else {
                bodyAnim = func_08034100(gMarchingOrders->marchersPointing
    ...
    // The remix version draws the head as part of the body, so it is never
    ...
    marcher->currentAction = action;
    marcher->idleTimer = newIdleTimer;

    // The original ends with a play_sound_w_pitch_volume behind a constant-zero
    // test, so it never runs. Not reproduced.
}
#endif
