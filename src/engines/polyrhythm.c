...
#else
// C translation of asm_08036428.s. Scans a lane for the next block after
// `start` that has a state set, and reports 15 when there is none - which is
// what stops a rod short of the end of the line.
s32 func_08036428(u32 lane, s32 start) {
    s32 i;

    if (start < 0) {
        start = 0;
    }
    for (i = start + 1; i <= POLYRHYTHM_BLOCK_AMOUNT - 1; i++) {
        if (gPolyrhythm->lanes[lane][i].state != 0) {
            return i;
        }
    }
    return POLYRHYTHM_BLOCK_AMOUNT - 1;
}
#endif
