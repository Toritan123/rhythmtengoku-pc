...
#else
// NOT TRANSLATED. Cue-barely reaction, still assembly upstream.
// The rest of this engine is C and runs; this one is a placeholder so the
// PC build links, and its absence is visible in-game rather than silent.
void func_08040314() {
}
#endif
...
#else
// NOT TRANSLATED. Per-frame fox position update, still assembly upstream.
// The rest of this engine is C and runs; this one is a placeholder so the
// PC build links, and its absence is visible in-game rather than silent.
void func_08040718() {
}
...
void tram_pauline_common_display_text(const char *text) {
    ...
    if (text == NULL) {
        ...
        // tram_pauline_engine_start is one of the nine functions this engine
        // still has in assembly, so on PC nothing ever creates the printer.
        // Skip rather than fault; the scene is visibly incomplete either way.
        if (gTramPauline->font == NULL) {
            return;
        }
