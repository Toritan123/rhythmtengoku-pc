...
void start_campaign_notice(s32 id) {
    ...
    string = notice->text;
    memcpy(string, "�������܁u", 11); // [Right now]
    strcat(string, level->name); // "<game_name>"
    strcat(string, "�v�Ńp�[�t�F�N�g��B�������"); // Get a perfect on this
    if (!isSpecialSong) {
        strcat(string, "����Ȃ�"); // game, and you'll receive
    }
    strcat(string, "�u"); // "
    strcat(string, get_campaign_gift_title(id, FALSE)); // "<gift>"
    strcat(string, "�v"); // "
    if (isStandardSong) {
        strcat(string, "�̋�"); // 's song
    }
    strcat(string, "���v���[���g!!"); // received as a present!!
